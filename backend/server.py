...
async def xero_oauth_start(body: Dict[str, Any], ctx: RequestContext = Depends(require_role("ADMIN"))):
    ...
    if XERO_MODE == "live":
        ...
        mock_url = f"https://finance-hub-225.preview.emergentagent.com/api/mock/xero/consent?state={state}"
...
@api.post("/billing/start-lite-trial")
async def start_lite_trial(ctx: RequestContext = Depends(require_role("ADMIN"))):
    if not ctx.org_id:
        raise HTTPException(status_code=400, detail="No org selected")
    now = datetime.now(timezone.utc)
    # Check if already on LITE or better
